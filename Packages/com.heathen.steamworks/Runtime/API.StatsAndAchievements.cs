    ...
    public static class StatsAndAchievements
    {
        ...
        public static class Client
        {
            ...
            static void Init()
            {
                ...
                loadedImages = new();
                eventUserStatsReceived = new();
                eventUserStatsUnloaded = new();
                eventUserStatsStored = new();
                eventUserAchievementStored = new();
                eventAchievementStatusChanged = new();
            ...
            private static void HandleUserStatsReceived(UserStatsReceived_t arg)
            {
                if (arg.m_nGameID == GameData.Me
                    ...
                    _userStatsLoaded = arg;

                eventUserStatsReceived.Invoke(arg);
            }
            /// <summary>
            ...
            public static UnityEvent<string, bool> EventAchievementStatusChanged
            {
                get
                {
                    return eventAchievementStatusChanged;
                }
            }

            private static List<ImageRequestCallbackLink> pendingLinks = new();
            private static Dictionary<int, Texture2D> loadedImages = new();
            private static UserStatsReceivedEvent eventUserStatsReceived = new();
            private static UserStatsUnloadedEvent eventUserStatsUnloaded = new();
            private static UserStatsStoredEvent eventUserStatsStored = new();
            private static UserAchievementStoredEvent eventUserAchievementStored = new();
            private static UnityEvent<string, bool> eventAchievementStatusChanged = new();
            ...
            private static UserStatsReceived _userStatsLoaded = new();
            public static UserStatsReceived UserStatsLoaded => _userStatsLoaded;
            ...
            public static bool ClearAchievement(string achievementApiName)
            {
                ...
                if(value)
                {
                    if (GetAchievement(achievementApiName, out bool status))
                        EventAchievementStatusChanged?.Invoke(achievementApiName, status);
                    else
                        EventAchievementStatusChanged?.Invoke(achievementApiName, false);
                }
            ...
            public static void GetMostAchievedAchievements(Action<EResult, (AchievementObject achievement, float percentage)[], bool> callback)
            {
                ...
                {
                    Debug.LogError($"{nameof(GetMostAchievedAchievements)} only works when you have initialized a {nameof(SteamSettings)} object");
